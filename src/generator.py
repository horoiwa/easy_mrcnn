...
from .constant import (
    N_TRAIN,  DATA_GEN_ARGS, IMAGE_COLORMODE,
    MASK_COLORMODE, PCA_COLOR, PCA_COLOR_RANGE,
...
def dataset_generator(dataset_dir, outdir):
    gen_test_dir = os.path.join(outdir, 'train')
    ...
    customGen = ImageMaskGenerator(batch_size=9999,
    ...
    for cnt in range(1, N_TRAIN+1):
        ...
        for n, (image, mask) in enumerate(zip(images, masks)):
            ...
            image.save(os.path.join(image_dir, f'{cnt}{n}.jpg'))

            if MASK_COLORMODE == 'RGB':
                raise NotImplementedError

            elif MASK_COLORMODE == 'L':
                mask = mask.reshape(TARGET_SIZE)*255
...
def ImageMaskGenerator(batch_size, dataset_dir, folder, aug_dict,
    ...
    for images, masks in imagemaskGen:
        ...
        if MASK_COLORMODE == 'L':
            masks_new = np.zeros((n_images,) + TARGET_SIZE + (1,))
        elif MASK_COLORMODE == 'RGB':
            raise NotImplementedError
